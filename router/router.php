...
class Router {
  ...
  public function map($rule, $target=array(), $conditions=array()) {
   $this->routes[$rule] = new Route($rule, ($this->uri_filter ? str_replace($this->uri_filter,'',$this->request_uri) : $this->request_uri), $target, $conditions);
  }
  ...
  private function set_route($route) {
    ...
    $this->action = (isset($params['action']) || isset($_GET['action']) ? (isset($_GET['action']) ? $_GET['action'] : $params['action']) : ROUTER_DEFAULT_ACTION); unset($params['action']);
